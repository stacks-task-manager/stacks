...
interface IBOStacksEstimationsProps {
    stacks: IStack[];
    value?: number;
    currency?: string;
}
...
export default class BOStacksEstimations extends React.PureComponent<IBOStacksEstimationsProps> {
    render() {
                    ...
                    <Tooltip
                        formatter={(
                            ...
                            if (entry.dataKey === "spent" && entry.payload.total - entry.payload.spent < 0) {
                                return [`${name}: ${entry.payload.spentTime || 0}`];
                            }

                            if (entry.dataKey === "spent") {
                                return [`${translate("Total Spent")}: ${entry.payload.spentTime || 0}`];
                            }

                            return [`${name}: ${entry.payload.totalTime || 0}`];
                        }}
                    />
                    <Bar
                        dataKey="total"
