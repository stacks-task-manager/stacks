...
export const PeopleRoles = () => {
    ...
    return (
        <AppViewContent relative padded data-testid="people-roles-view">
            <div className="people-roles" data-testid="people-roles-content">
                <TablePersistent
                    id="roles"
                    sticky
            ...
            >
                <div className={Classes.DIALOG_BODY}>
                    ...
                    <HTMLTable striped>
                        ...
                        <tbody>
                            {(Object.keys(ROLE_SECTIONS) as Array<keyof typeof ROLE_SECTIONS>).map(
                                section => {
                                    ...
                                    return (
                                        <tr key={section}>
                                            <td>
                                                <Icon icon={ROLE_ICONS[ROLE_SECTIONS[section]]} />{" "}
                                                {ROLE_SECTIONS[section]}
                                            </td>
                                            <td>
                                                <Checkbox
                                                    data-testid={`role-access-${ROLE_SECTIONS[section]}-read`}
                                                    checked={read}
                                                    disabled={ROLE_SECTIONS[section] === ROLE_SECTIONS.ROLES}
                                                    onChange={() =>
                                                        handleUpdateAccess(ROLE_SECTIONS[section], {
                                                            read: !read,
                                                            write,
                                                        })
                                                    }
                                            ...
                                            <td>
                                                <Checkbox
                                                    data-testid={`role-access-${ROLE_SECTIONS[section]}-write`}
                                                    checked={write}
                                                    disabled={
                                                        [ROLE_SECTIONS.ROLES, ROLE_SECTIONS.REPORTS].includes(
                                                            ROLE_SECTIONS[section]
                                                        ) || !read
                                                    }
                                                    onChange={() =>
                ...
                <div className={Classes.DIALOG_FOOTER}>
                    <div className={Classes.DIALOG_FOOTER_ACTIONS}>
                        ...
                        <Button
                            data-testid="role-edit-save-button"
...
const RoleCell: FC<TablePersistentCellProps<IRole>> = ({ row, column, callback }) => {
    if (column === "access") {
        ...
        return (
            <Button size="small" variant="minimal" icon={<Icon icon="edit-05" />} onClick={callback}>
                Edit role
            </Button>
        );
    }

    return <span>{String(row[column as keyof IRole] ?? "")}</span>;
};
