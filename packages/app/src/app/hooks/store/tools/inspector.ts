...
/**
 * Redux-style inspector for Immer entities.
 */
...
declare global {
    interface Window {
        __REDUX_DEVTOOLS_EXTENSION__?: {
            connect(options: { name?: string; features?: Record<string, any> }): DevToolsConnection;
        };
        __inspector?: {
...
/** Clears inspector singleton state — for unit tests only (e.g. after toggling mocked DevTools). */
export function __resetInspectorForTests() {
    devTools = null;
    isInspectorEnabled = false;
    isInspectorInitialized = false;
    isDevToolsInitialized = false;
    isDevToolsPaused = false;
    initialRegistryValue = null;
    // Keep window hook aligned with isInspectorEnabled so registry `entity().init()` does not
    // call onInit(__GLOBAL__) mid-setup while tests toggle mocks between runs.
    window.__inspector = undefined;
}
