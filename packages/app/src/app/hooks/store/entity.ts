...
export function entity<T = any>(initialValue: T, pluginsOrAlias?: Plugin[] | string): Entity<T>;

export function entity(initialValue: any, pluginsOrAlias: Plugin[] | string = []): Entity {
...
function createInit(entity: EntityImpl, initialValue: any): Entity["init"] {
    return () => {
        ...
        if (initialValue instanceof Promise)
            ...
            initialValue.then((value: any) => setTimeout(() => entity.set!(value, "@@ASYNC_INIT")));
...
function applyPlugins(entity: Entity, plugins: Plugin[]) {
    plugins.forEach(plugin => {
        ...
        Object.keys(plugin).forEach(method => overrideMethod(method as keyof Plugin));
    });
}
