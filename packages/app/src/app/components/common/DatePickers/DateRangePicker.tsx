...
import { DateRangePicker as BlueprintDateRangePicker, DateRange, TimePicker } from "@blueprintjs/datetime";
...
import { FormattedDateRange, getSingleDateShortcuts, useDateIntent } from "./utils";
...
export const DateRangeComponent: FunctionComponent<DateRangePickerProps> = ({
    ...
    const shortcuts: IDatePickerShortcutExtra[] = useMemo(
        () => getSingleDateShortcuts(endOfDay(new Date())),
        []
    );
        ...
        return [
            {
                ...
                dateRange: [resetTime(startOfWeek(now)), resetTime(endOfWeek(now))],
            },
            {
                title: translate("Next week"),
                label: `${format(startOfWeek(addWeeks(now, 1)), "d")} - ${format(
                    endOfWeek(addWeeks(now, 1)),
                    "d MMM"
                )}`,
                dateRange: [resetTime(startOfWeek(addWeeks(now, 1))), resetTime(endOfWeek(addWeeks(now, 1)))],
            },
            {
                title: translate("This month"),
                label: `${format(startOfMonth(now), "d")} - ${format(endOfMonth(now), "d MMM")}`,
                dateRange: [resetTime(startOfMonth(now)), resetTime(endOfMonth(now))],
            },
            {
                title: translate("Next month"),
                label: `${format(startOfMonth(addMonths(now, 1)), "d")} - ${format(
                ...
                <BlueprintDateRangePicker
                    ...
                    footerElement={
                        <Grid>
                            ...
                            <FormattedDateRange
                                range={value}
                                showTime={enableTimePicker}
                                onClearStart={() => setValue([null, value[1]])}
                                onClearEnd={() => setValue([value[0], null])}
                            />
