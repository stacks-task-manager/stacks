...
async function incrementVersion(type = "patch") {
    try {
        ...
        if (versionParts.length !== 3) {
            throw new Error(`Invalid version format: ${packageJson.version}. Expected MAJOR.MINOR.PATCH`);
        }
        ...
        console.log(`✅ ${type.charAt(0).toUpperCase() + type.slice(1)} version incremented successfully!`);
        console.log(`📈 New version: ${newVersion}`);
    } catch (error) {
        console.error(`❌ Failed to increment ${type} version:`, error);
        process.exit(1);
    }
}
